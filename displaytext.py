...
def display_text(text, size, font, color, bgcolor=(30,30,30), italic=False, bold=False, underline=False, margin=1.25, rotation=0):
    pygame.font.init()
    torender = pygame.font.Font(font, size)
    torender.set_italic(italic); torender.set_bold(bold); torender.set_underline(underline) # Setting all of the parameters
    MarginSurf, TextSurf, TextRect = text_objects(text, torender, color, bgcolor, margin, rotation)
    TextRect.center = (0, 0)
    return MarginSurf, TextSurf, TextRect

def display_text_paragraph(surface, referencePoint, text, size, font, italic=False, bold=False, underline=False, margin=1.25, centered=False, color=(0,0,0), bgcolor=(30,30,30), rotation=0):
    pygame.font.init()
    ...
    for line in textlist:
        marginSurf, textSurf, textRect = display_text(line, size, font, color, bgcolor, italic, bold, underline, margin, rotation) # Now getting the y increment from this
...
def display_text_alpha(text, size, font, color, alpha=255, bgcolor=(30,30,30), italic=False, bold=False, underline=False, margin=0.25, rotation=0): # Margin is just multiplying the width and height by a decimal 
    pygame.font.init()
    torender = pygame.font.Font(font, size)
    torender.set_italic(italic); torender.set_bold(bold); torender.set_underline(underline) # Setting all of the parameters
